...
| `data/kb-index.json` | 23,337 BM25 index chunks. |
| `library/` | SharePoint docs + the H&S reference library (IRIS/Odyssey/DSE) mirrored on Pages. |

## Data State
- **Current total: 6,680 documents, 23,337 index chunks** — verified directly against live `data/kb.json`/`data/kb-index.json` via the git blob API (both files exceed the ~1MB Contents API ceiling), 18 September 2026 (Adam). Per-source breakdown: Cority 4,092, Access Group Help Centre 2,251, How To Guides 209, Oxford IT Sign-In Directory 53, Change Management 51, IRIS 6, Kevin's Guides 5, HRIS Launcher (PeopleXD) 5, Odyssey 4, DSE 4. This file had drifted (stated 6,688/23,338, dated 19 Aug) — corrected here; full narrative in `HANDOVER.md`'s 18 Sep entry, including a known indexing bug (`MAX_CHUNKS_PER_DOC=40` silently truncates long PDFs) and a real still-open KB-card wiring gap (Colleges & Halls guide).
