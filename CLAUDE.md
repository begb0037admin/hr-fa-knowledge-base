...
- **Status:** Active — 6,680 documents across PeopleXD (Access Group/SharePoint/How To Guides/Kevin's Guides), Health & Safety (Cority, IRIS, Odyssey, DSE), a SERVICES section (Oxford IT Sign-In Directory, HRIS Launcher/PeopleXD Service Catalogue+Other Teams+Data Protection), voice search (mic + Listen, push-to-talk, Cloudflare Workers AI), cross-session memory (Linda Option C — deployed and live, `MEM` KV binding, `hr-kb-ai` Worker)
...
| `scrapers/` | `access_group_scraper.py` (Playwright, public help centres, `--deep` for full harvest), `cority_clickhelp_scraper.py` (Cority ClickHelp corpus, no login needed), `build_index.py` (merges all sources → `data/kb.json` + `data/kb-index.json`), `extract_sharepoint.py` (unpacks SP zips → `library/`), `extract_hs_library.py` (extracts text from the curated IRIS/Odyssey/DSE reference library → `library/Health and Safety/`). |
| `data/kb.json` | Knowledge base cards (6,680 documents). |
| `data/kb-index.json` | 23,345 BM25 index chunks. |
| `library/` | SharePoint docs + the H&S reference library (IRIS/Odyssey/DSE) mirrored on Pages. |

## Data State
- **Current total: 6,680 documents, 23,345 index chunks** — verified directly against live `data/kb.json`/`data/kb-index.json` via the git blob API (both files exceed the ~1MB Contents API ceiling), 19 September 2026 (Adam). Chunk count moved from 23,337 to 23,341 that session (4 cross-reference chunks); it then moved to 23,345 later on 19 Sep 2026 (commit `32a465c8`, Linda's text for the Colleges & Halls guide refreshed from the real .docx, doc 2514: 7 to 11 chunks). Earlier that day: 4 new cross-reference chunks added to the Data Migration Tool and UDF Configuration guides (doc idx 63/422/192/319), no new document. Per-source breakdown (documents, unchanged by this session): Cority 4,092, Access Group Help Centre 2,251, How To Guides 209, Oxford IT Sign-In Directory 53, Change Management 51, IRIS 6, Kevin's Guides 5, HRIS Launcher (PeopleXD) 5, Odyssey 4, DSE 4. Full narrative in `HANDOVER.md`'s 19 Sep entry (and 18 Sep before it), including a known indexing bug (`MAX_CHUNKS_PER_DOC=40` silently truncates long PDFs), a still-open KB-card wiring gap (Colleges & Halls guide), and a new technical-debt item (no durable annotation layer for scraped docs, see `ROADMAP.md`).
...
- This file has drifted repeatedly and been caught the same way each time — by verifying live data rather than trusting either document's prose: before the 31 July check (stated 2,226 documents / 7,230 chunks, last touched 18 June, while `HANDOVER.md` already had the real figure), again after the Cority build (still said "not yet built" after Cority had shipped), and again by 18 September 2026 (stated 6,688/23,338 from 19 Aug while three weeks of real commits, including new Kevin's Guides content, had landed on `main` with no `HANDOVER.md` entry at all — see that file's 18 Sep entry). If this file and `HANDOVER.md` ever disagree, trust `HANDOVER.md` and re-verify against the live data files — and don't assume `HANDOVER.md` itself is current either; check its own "From" date against real `git log` first.
