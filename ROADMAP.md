...
- **This is actually two separate pieces of work, worth splitting rather than treating as one task:**
  1. **Cross-session conversation memory** — so Linda can recall a specific past conversation with a specific person. Needs persistent storage (e.g. Cloudflare KV or D1, bound to the Worker) keyed per user/session, written after each exchange, loaded back in on the next visit. **Approved direction — Option C: server-side per-identity store, Cloudflare KV bound to the `hr-kb-ai` Worker, keyed on a hash of `KB_ACCESS_TOKEN` (decided by Kevin, 27 August 2026). NOT built. Full pick-up-ready build plan — mechanism, storage/identity rationale, data model, UI changes, deploy sequence with restore point + rollback, test plan: [`LINDA-OPTION-C-BUILD-PLAN.md`](LINDA-OPTION-C-BUILD-PLAN.md).**
  2. **Self-learning / compounding from usage** — a separate, larger capability: Linda getting better over time from the pattern of questions asked (e.g. surfacing frequently-asked things the KB covers poorly, or common phrasing that should map to a specific document). Genuinely different work from #1 — needs its own design before it can be scoped: what "compounding" concretely means (a running FAQ digest? weighting search results by past query success? something else?) hasn't been decided anywhere findable. **Still needs scoping — the open questions a scoping session must answer (what qualifies as worth learning, curation/review, correcting wrong learnings, keeping it from polluting the curated KB, versioning, relationship to Option C) are written up pick-up-ready in [`LINDA-OPTION-D-SCOPING-BRIEF.md`](LINDA-OPTION-D-SCOPING-BRIEF.md).**
- **Status (28 August 2026):** Item 1 — **Option C BUILT & DEPLOYED — LIVE.** Worker `hr-kb-ai` v`6e026fed` has the `/memory` route + `MEM` KV binding (namespace `linda-conversation-memory`); identity is a fixed key `mem:v1:primary` (`MEM_IDENTITY` var override) — one shared history, zero-config on any machine (superseded the plan's token/device-id identity, Kevin's 28 Aug call). Client on `main`. Rollback = `wrangler rollback 36e78505-f567-4986-9299-23e86e18f12f --name hr-kb-ai`. Item 2 — still needs scoping. **A candidate architecture is now recorded** in [`LINDA-OPTION-D-SCOPING-BRIEF.md`](LINDA-OPTION-D-SCOPING-BRIEF.md) → "Candidate architecture — the 'AI librarian' second-brain pattern" (Karpathy-style `raw/`→`wiki/`→`outputs/` curated layer + monthly health-check skill, Adam-led with Markey). **Kevin's explicit instruction 28 Aug 2026: this must be picked up at the next KB discussion — do not let it be forgotten.** No design or build until Kevin answers the brief's open questions.
...
- **SERVICES sidebar section — Oxford IT Sign-In Directory + HRIS Launcher (PeopleXD) Service Catalogue/Other Teams/Data Protection (19 August 2026, session 6).** Kevin asked for the sign-in directory (built 18 Aug but left unwired) to be added to a "Services" section, alongside "other teams," "data protection," and "some additional headings." Resolved via evidence: `pxd.lelitte.co.uk` (`hris-launcher`) turned out to be the only real source of literal "Services"/"Other Teams"/"Data Protection" headings, fetched directly and mirrored in as 14 new curated records (`data/pxd-services.json`). New `SERVICES` sidebar section in `index.html` with two expandable sources (Oxford IT Sign-In Directory, 53 docs; HRIS Launcher (PeopleXD), 14 docs across 3 topic groups), following the established Cority-style pattern. Kevin reviewed the live section and approved it verbally (19 Aug 2026) — closing the visual-approval follow-up from this item. Full verification chain and the IA judgement call are in `HANDOVER.md` → session 6.
- **9 dead HRIS Launcher URLs fixed at the source; KB re-synced (19 August 2026, session 7).** The 9 URLs flagged as dead in session 6 (all 8 "Service Catalogue" entries + "Reward | Personnel Services") were removed directly from `begb0037admin/hris-launcher/index.html` — the actual live sidebar `data/pxd-services.json` was mirrored from in the first place — under a one-off Kevin-authorized exception to Adam's normal repo scope. `data/pxd-services.json` in this repo trimmed from 14 to 5 records. The now-empty "Service Catalogue" `tp` group needed no explicit removal code — `index.html`'s existing `pxdMods` derivation only ever renders `tp` keys with ≥1 live record, confirmed by reading the code rather than assuming. Also mirrored the SERVICES section back onto `pxd.lelitte.co.uk` itself: added a new "Oxford IT Sign-In Directory" nav-group (53 entries, static HTML matching that repo's existing no-build-step pattern) to `hris-launcher/index.html`. Full verification chain (git blob diffs, live-site byte comparisons, both Pages deployments polled to completion) in `HANDOVER.md` → session 7. `hris-launcher/RESUME.md` created as that repo's first durable state record.
